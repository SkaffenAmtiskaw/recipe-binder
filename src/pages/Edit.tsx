...
const Edit: FunctionComponent<Props> = ({ recipes, user }) => {
  ...
  const [ingredients, setIngredients] = useState<string[]>(get(recipe, 'ingredients', []));
  const [instructions, setInstructions] = useState<string[]>(get(recipe, 'instructions', []));
  const [notes, setNotes] = useState<string>(get(recipe, 'notes', ''));
  const [servings, setServings] = useState<number | undefined>(get(recipe, 'servings'));
  const [source, setSource] = useState<{ name?: string, url?: string }>(get(recipe, 'source', {}));
  const [storage, setStorage] = useState<string>(get(recipe, 'storage', ''));
  const [tags, setTags] = useState<string[]>(get(recipe, 'tags', []));
  const [time, setTime] = useState<{ prep?: string, cook?: string, total?: string }>(get(recipe, 'time', {}));
